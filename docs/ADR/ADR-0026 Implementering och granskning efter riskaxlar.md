...
Samma sak gäller `fraga:arkitekt`-banan: beviljar arkitekten ett undantag på en köns egen, redan godkända PR, mergas den när CI blir grön, i stället för att stå kvar på `needs-human`. Mergen är dessutom bunden till den commit som granskades (`gh pr merge --match-head-commit`). Kön pausar också i stället för att bränna försök när testsviten är röd redan på main (`sviten_ar_rod_pa_basen()`); det är inte issuens fel, och issuen märks inte `needs-human`.
